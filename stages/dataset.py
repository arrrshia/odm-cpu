        ...
        def get_images(in_dir):
            ...
            for i, f in enumerate(entries):
                if valid_filename(f, context.supported_extensions):
                    ...
                    old_file_path = os.path.join(in_dir, f)
                    _, file_extension = os.path.splitext(f)

                    new_file_name = f"{i}{file_extension}"  # Change the extension if needed
                    new_file_path = os.path.join(in_dir, new_file_name)

                    # Rename the file
                    os.rename(old_file_path, new_file_path)

                    # Add the new file name to the valid list
                    valid.append(new_file_name)
                else:
                    rejects.append(f)
